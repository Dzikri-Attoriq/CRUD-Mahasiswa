...
<div class="my-5">
    <div class="container mx-auto max-w-xl shadow py-4 px-10">
        ...
        <div class="my-3">
            <h1 class="text-center text-3xl font-bold">Edit Jurusan</h1>
            <form action="/jurusan/{{ $data_jurusan->nama_jurusan }}" method="POST">
                @csrf
                @method('PUT')
                <div class="my-2 ">
                    <label for="nama_jurusan" class="text-md font-bold">Nama Jurusan</label>
                    <input type="text" name="nama_jurusan" id="nama_jurusan" value="{{ old('nama_jurusan', $data_jurusan->nama_jurusan) }}" class="block w-full border border-emerald-500 outline-emerald-800 px-2 py-2 text-md rounded-md my-2" id="">
                    @error('nama_jurusan')
