...
<div class="my-5">
    <div class="container mx-auto max-w-xl shadow py-4 px-10">
        ...
        <div class="my-3">
            <h1 class="text-center text-3xl font-bold">Edit Mahasiswa</h1>
            <form action="/mahasiswa/{{ $data_mahasiswa->nim }}" method="POST" enctype="multipart/form-data">
                @csrf
                @method('PUT')

                <div class="my-2 ">
                    <label for="nim" class="text-md font-bold">NIM</label>
                    <input type="text" name="nim" id="nim" value="{{ old('nim', $data_mahasiswa->nim) }}" class="block w-full border border-emerald-500 outline-emerald-800 px-2 py-2 text-md rounded-md my-2">
                    @error('nim')
                ...
                <div class="my-2 ">
                    ...
                    <input type="text" name="nama_mahasiswa" id="nama_mahasiswa" value="{{ old('nama_mahasiswa', $data_mahasiswa->nama_mahasiswa) }}" class="block w-full border border-emerald-500 outline-emerald-800 px-2 py-2 text-md rounded-md my-2">
                ...
                <div class="my-2">
                    ...
                    <select id="jenis_kelamin" name="jenis_kelamin" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
                        ...
                        <option value="Laki-laki" {{ old('jenis_kelamin', $data_mahasiswa->jenis_kelamin) == 'Laki-laki' ? 'selected' : '' }}>Laki-laki</option>
                        <option value="Perempuan" {{ old('jenis_kelamin', $data_mahasiswa->jenis_kelamin) == 'Perempuan' ? 'selected' : '' }}>Perempuan</option>
                    </select>
                    @error('jenis_kelamin')
                    <span class="text-red-500">{{$message}}</span>
                    @enderror
                    ...
                    <select id="jurusan_id" name="jurusan_id" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
                        ...
                        @foreach ($jurusans as $jurusan)
                            <option value="{{ $jurusan->id }}"
                                {{ old('jurusan_id', $data_mahasiswa->jurusan_id) == $jurusan->id ? 'selected' : '' }}>
                ...
                <div class="my-2 ">
                    ...
                    <input type="text" name="no" id="no" value="{{ old('no', $data_mahasiswa->no) }}" class="block w-full border border-emerald-500 outline-emerald-800 px-2 py-2 text-md rounded-md my-2">
                    @error('no')
                    <span class="text-red-500">{{$message}}</span>
                    @enderror
                </div>
